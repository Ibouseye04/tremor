...
// Internal: shared implementation to avoid type cycles

async function generateSummaryImpl(ctx: any): Promise<any> {
...
export const generateSummary: any = internalAction({
  args: {},
  handler: async (ctx) => generateSummaryImpl(ctx),
});
...
export const generateSummaryCron = internalAction({
  ...
  handler: async (ctx) => {
    ...
    const result = await generateSummaryImpl(ctx);
    logger.info(
...
export const refreshIfNeeded: any = action({
  args: {},
  handler: async (ctx) => {
    const current: any = await ctx.runQuery(api.marketSummary.getSummary);
    ...
    if (!current || now - current.generatedAt > THIRTY_MINUTES) {
      return await generateSummaryImpl(ctx);
    }

    return { success: true, cached: true, summary: current } as any;
  },
});

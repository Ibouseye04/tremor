...
// US state centroids and abbreviations (subset kept comprehensive for MVP)
...
// Country centroids with common aliases (same coverage as client MVP)
const COUNTRIES: Record<
  string,
  { code: string; lat: number; lng: number; aliases?: string[] }
> = {
  ...
  Netherlands: { code: 'NL', lat: 52.1326, lng: 5.2913, aliases: ['holland'] },
...
function matchUSState(
  text: string
): { region: string; lat: number; lng: number } | null {
  const nameMatch = text.match(STATE_NAME_REGEX);
  if (nameMatch) {
    const stateName = normalizeStateName(nameMatch[1]);
    const byName = lookupState(stateName);
    if (byName) return { region: stateName, lat: byName.lat, lng: byName.lng };
  }
  const abbrMatch = text.match(STATE_ABBR_REGEX);
  if (abbrMatch) {
    const abbr = abbrMatch[1].toUpperCase();
    const stateName = ABBR_TO_STATE[abbr];
    const byAbbr = lookupState(stateName);
    if (byAbbr) return { region: stateName, lat: byAbbr.lat, lng: byAbbr.lng };
  }
  return null;
}

function lookupState(stateName?: string) {
  if (!stateName) return null;
  const s = US_STATES[stateName];
  if (!s) return null;
  return { lat: s.lat, lng: s.lng, region: stateName, country: 'US' } as const;
}
...
function matchCountryByName(
  fragment: string
): Omit<ServerGeo, 'confidence'> | null {
...
export function enrichGeoFromText(
  title: string,
  question?: string
): ServerGeo | null {
  const titleN = normalize(title || '');
  const questionN = normalize(question || '');
  const text = `${titleN} ${questionN}`.trim();

  // Exclude generic global/federal terms (do not attach a misleading point)
  if (/(\b(fed|federal|nationwide|global|world)\b)/i.test(text)) {
    return null;
  }

  // 1) Office patterns (US-centric)
  ...
  if (officeMatch) {
    ...
    const byStateName = matchUSState(locationStr);
    if (byStateName)
      return { ...lookupState(byStateName.region)!, confidence: 'high' };
    const byCountryFromOffice =
      matchCountryByName(locationStr) || matchCountryByAlias(locationStr);
    if (byCountryFromOffice)
      return { ...byCountryFromOffice, confidence: 'high' };
  }

  // 2) Generic in/at/of pattern
  ...
  if (inMatch) {
    ...
    if (byStateName)
      return { ...lookupState(byStateName.region)!, confidence: 'high' };
    const byCountryName = matchCountryByName(locationStr);
    if (byCountryName) return { ...byCountryName, confidence: 'high' };
    const byCountryAlias = matchCountryByAlias(locationStr);
    if (byCountryAlias) return { ...byCountryAlias, confidence: 'medium' };
  }

  // 3) State anywhere
  const sMatch = text.match(STATE_NAME_REGEX);
  if (sMatch) {
    const st = normalizeStateName(sMatch[1]);
    const byName = lookupState(st);
    if (byName) return { ...byName, confidence: 'high' };
  }

  // 4) Country anywhere
  const byCountryAnywhereName = matchCountryByName(text);
  if (byCountryAnywhereName)
    return { ...byCountryAnywhereName, confidence: 'high' };
  const byCountryAnywhereAlias = matchCountryByAlias(text);
  if (byCountryAnywhereAlias)
    return { ...byCountryAnywhereAlias, confidence: 'medium' };
